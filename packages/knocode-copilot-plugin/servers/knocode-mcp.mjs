...
async function handleLine(req) {
    ...
    if (!resp) {
        ...
        if (!notif) {
            process.stdout.write(errorEnvelope(req?.id, -32000, `knocode daemon unreachable at ${DAEMON_URL} — start it with \`knocode serve\``) + "\n");
        }
        return;
    }
    if (!resp.ok) {
        ...
        process.stdout.write(errorEnvelope(req?.id, -32002, message) + "\n");
        return;
    }
    if (notif)
        return; // forwarded, but never answered
    ...
    rl.on("line", (line) => {
        ...
        handleLine(req).catch(() => {
            if (!isNotification(req)) {
                process.stdout.write(errorEnvelope(req?.id, -32603, "internal knocode-mcp error") + "\n");
            }
        });
    });
}
