...
async function handleToolsCall(name, args) {
    ...
    if (name === "coderun_search" || name === "coderun_preview") {
        ...
        if (candidate_k)
            process.env.CODERUN_CANDIDATE_K = String(candidate_k);
        if (max_files)
            process.env.CODERUN_MAX_FILES = String(max_files);
        const req = {
        ...
        if (candidate_k) {
            if (prevEnvK)
                process.env.CODERUN_CANDIDATE_K = prevEnvK;
            else
                delete process.env.CODERUN_CANDIDATE_K;
        }
        if (max_files) {
            if (prevEnvM)
                process.env.CODERUN_MAX_FILES = prevEnvM;
            else
                delete process.env.CODERUN_MAX_FILES;
        }
        if (!res)
            return { content: [{ type: "text", text: "Coderun daemon unreachable at " + DAEMON_URL + " — run coderun serve" }] };
        const rewritten = res?.payload?.rewritten || res?.payload?.reason || JSON.stringify(res, null, 2);
        const pack = res?.payload?.context_pack;
    ...
    if (name === "coderun_symbols") {
        ...
        const res = await callDaemon(req);
        if (!res)
            return { content: [{ type: "text", text: "daemon unreachable" }] };
...
async function main() {
    ...
    rl.on("line", async (line) => {
        ...
        const id = req.id;
        let response = { jsonrpc: "2.0", id, result: null };
        try {
            switch (req.method) {
                case "initialize": {
                    response.result = {
