...
import { callKnocodeDaemon } from "../src/index";

/**
 * Canonical E2E: OpenCode → Knocode (UDS/HTTP fallback) → BuildContext → response
...
describe("E2E: OpenCode → Knocode → BuildContext", () => {
  it("enriches message via BuildContext (mocked daemon, no Router/LiteLLM)", async () => {
    ...
    const resp = await callKnocodeDaemon(
