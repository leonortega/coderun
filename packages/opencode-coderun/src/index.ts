...
export function getReadyTimeoutMs(): number {
  const raw = process.env.CODERUN_READY_TIMEOUT_MS;
  if (raw) {
    const n = Number(raw);
    if (Number.isFinite(n) && n > 0) return n;
  }
...
/**
 * Call the Coderun daemon HTTP `/hook` endpoint.
...
// drive Coderun with typed tools (`tools/call`) instead of shaping prompts/answers
...
/**
 * Pre-model context enrichment: drive `coderun_context` via MCP. Returns the enriched
...
export async function requestContextEnrichment(
  ...
  switch (out.kind) {
    ...
    case "unsupported": {
      ...
      const response = await callCoderunDaemon(request, opts);
...
/**
 * Tool-output compression: drive `coderun_compress` via MCP. Returns the compressed
...
export const CoderunPlugin: Plugin = async ({ project, client, $, directory, worktree }: any) => {
  ...
  async function ensureDaemonReady(): Promise<void> {
    ...
    if (ready && Date.now() - waitStartedAt > 1_000) {
      console.log(`[coderun] Daemon became ready after ${Date.now() - waitStartedAt}ms`);
    }
  }
  ...
  async function enrichMessage(input: any): Promise<void> {
    ...
    if (rewritten != null) {
      ...
      console.log("[coderun] Enriched message");
    }
