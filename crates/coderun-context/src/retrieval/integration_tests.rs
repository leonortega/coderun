...
//! ## Configuration
//!
//! Set `CODERUN_TEST_REPO=/path/to/repo` to run tests against a real codebase.
//! Without it, tests use a built-in temp fixture with hardcoded test files.
//!
//! ```bash
//! # Default: built-in fixture
//! cargo test -p coderun-context retrieval::integration_tests
//!
//! # Real repo (e.g. DefinitelyTyped)
//! CODERUN_TEST_REPO=/c/tmp/DefinitelyTyped-master \
//!   cargo test -p coderun-context retrieval::integration_tests
//! ```
...
use coderun_repo_intel::structural::AstGrepBackend;

// ── Test repository configuration ──────────────────────────────────────

/// Check for a real repository path via CODERUN_TEST_REPO env var.
fn test_repo_path() -> Option<PathBuf> {
    std::env::var("CODERUN_TEST_REPO")
        .ok()
        .filter(|p| !p.is_empty())
        .map(PathBuf::from)
        .filter(|p| p.is_dir())
...
fn integration_find_functions() {
    ...
    for tf in &files {
        let adapter = match coderun_repo_intel::structural::create_adapter(tf.language) {
            Some(a) => a,
            None => continue,
        };
...
fn integration_find_interfaces() {
    ...
    for tf in &files {
        ...
        let adapter = match coderun_repo_intel::structural::create_adapter("typescript") {
            Some(a) => a,
            None => continue,
        };
...
fn integration_negative_wrong_pattern_for_language() {
    let adapter = match coderun_repo_intel::structural::create_adapter("python") {
        Some(a) => a,
        None => return,
    };
...
fn integration_dt_find_interfaces_in_dts() {
    let repo = match test_repo_path() {
        Some(p) => p,
        None => return, // Only runs with CODERUN_TEST_REPO
    };
