...
fn eval_graph(
    ...
    for q in &queries {
        ...
        .unwrap_or_else(|_| {
            crate::retrieval::evidence::RetrievalResult::empty(coderun_core::RetrievalStatus::NoMatch)
        });
...
#[ignore]
fn bench_components() {
    let coderun_root = PathBuf::from("C:/LeonRepository/coderun");
    if !coderun_root.exists() {
        eprintln!("coderun repo not found at {:?} — skipping", coderun_root);
        return;
    }

    eprintln!("Running component evaluation on coderun repo...");
