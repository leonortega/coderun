...
use coderun_repo_intel::structural::{AstGrepBackend, TsLangAdapter, cached_adapter, ext_to_lang_pack_name};
// Re-export for structural_plan module
pub(crate) use coderun_repo_intel::structural::lang_patterns_for;
...
impl StructuralRetriever {
    ...
    pub fn retrieve_with_candidates(
        ...
        let status = if timed_out {
            coderun_core::RetrievalStatus::FallbackUsed("structural:timeout".to_string())
        } else {
            coderun_core::RetrievalStatus::Found(evidence_count)
        };
...
mod tests {
    use super::*;
    use coderun_repo_intel::structural::create_adapter;
...
"#;
    ...
    fn test_adapter_for_ext_uses_cache() {
        use coderun_repo_intel::structural::{clear_adapter_cache, adapter_cache_size};
        clear_adapter_cache();
