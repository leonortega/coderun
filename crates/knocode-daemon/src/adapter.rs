...
use knocode_optimizer::{ExecutionOptimizer, OptimizerConfig};
...
impl Default for AdapterConfig {
    fn default() -> Self {
        Self {
            socket_path: std::path::PathBuf::from("/tmp/knocode.sock"),
...
async fn handle_pre_generation(
    ...
    Ok(ResponsePayload::RewrittenMessage(Box::new(knocode_core::RewrittenMessageData {
        original: message.clone(),
        rewritten: format!(
            "{}\n\n---\n\nContext:\n{}",
            message,
            knocode_context::ContextEngine::to_yaml(&context_pack)?
        ),
...
mod tests {
    ...
    fn test_adapter_config_default() {
        ...
        assert_eq!(config.socket_path, std::path::PathBuf::from("/tmp/knocode.sock"));
    ...
    fn test_messagepack_roundtrip() {
        let request = AgentRequest {
            ...
            payload: RequestPayload::MessageRewrite {
                ...
                context_hints: Some(knocode_core::ContextHints {
    ...
    fn test_response_serialization() {
        let response = AgentResponse {
            correlation_id: CorrelationId::new(),
            hook_type: HookType::PreGeneration,
            payload: ResponsePayload::RewrittenMessage(Box::new(knocode_core::RewrittenMessageData {
    ...
    async fn test_handle_request_fail_open_on_build_context_error() {
        ...
        let repo_path = std::env::temp_dir().join(format!("knocode_adapter_test_{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&repo_path).unwrap();
        let repo_intel = knocode_repo_intel::RepositoryIntelligence::new(repo_path.clone(), knocode_storage::Database::open(&std::path::PathBuf::from(":memory:")).unwrap(), event_bus.clone());
        let kh = knocode_knowledge::KnowledgeHub::new(db, event_bus.clone(), knocode_knowledge::KnowledgeConfig::default());
        let engine = ContextEngine::new(repo_intel, kh, event_bus.clone(), knocode_context::ContextConfig::default());
