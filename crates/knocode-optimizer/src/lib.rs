...
use knocode_core::OutputType;
use rtk::RtkAdapter;
use tracing::{debug, warn};

/// Execution Optimizer — RTK binary (github.com/rtk-ai/rtk) primary when on PATH, built-in compressors fallback on Err with WARN
#[derive(Clone)]
...
impl ExecutionOptimizer {
    ...
    pub fn compress_output(
        ...
        // RTK binary (github.com/rtk-ai/rtk) is primary when on PATH; built-ins are fallback only on Err with WARN
            ...
            Err(_) => {
                ...
                let log_dir = {
                    if let Some(home) = dirs_home() {
                        home.join(".knocode").join("logs").join("tool-failures")
                    } else {
                        std::path::PathBuf::from(".knocode/logs/tool-failures")
                    }
                };
