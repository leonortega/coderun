...
//!   (b) tools/call coderun_context returns the enriched answer + structured metadata
//!   (c) zero-hit prompt → passthrough (text == prompt, passthrough: true)
//!   (d) readiness gate: coderun_context while indexing → JSON-RPC -32001 (HTTP 200)
//!   (e) tools/call coderun_compress shrinks repetitive output
...
use coderun_daemon::http_server::{create_router, HttpServerState};
...
fn temp_dir(tag: &str) -> PathBuf {
    std::env::temp_dir().join(format!("coderun_e2e_mcp_{}_{tag}", uuid::Uuid::new_v4()))
}
...
async fn e2e_mcp_contracts() {
    ...
    std::env::set_var("CODERUN_REPO_STATE", "e2e-mcp-head");
    ...
    assert_eq!(json["result"]["serverInfo"]["name"], "coderun");
    ...
    let names: Vec<String> = json["result"]["tools"]
        ...
        .collect();
    assert!(names.contains(&"coderun_context".to_string()), "{names:?}");
    assert!(names.contains(&"coderun_compress".to_string()), "{names:?}");
    ...
    // (b) coderun_context on a seeded hit → enriched text + structured metadata
    ...
    coderun_daemon::metrics::global().set_readiness(coderun_daemon::metrics::Readiness::Indexing);
    let (status, json) = post_mcp(
        &port,
        &serde_json::json!({
            "jsonrpc": "2.0",
            "id": 5,
            "method": "tools/call",
            "params": { "name": "coderun_context", "arguments": { "prompt": "hi", "repository_path": repo_path } }
        }),
    )
    .await;
    coderun_daemon::metrics::global().set_readiness(coderun_daemon::metrics::Readiness::Ready);
    ...
    // (e) coderun_compress shrinks repetitive output
