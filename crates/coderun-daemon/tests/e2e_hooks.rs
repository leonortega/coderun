...
//! index (`CODERUN_INDEX_DIR`) and asserts:
...
fn temp_dir(tag: &str) -> PathBuf {
    std::env::temp_dir().join(format!("coderun_e2e_{}_{tag}", uuid::Uuid::new_v4()))
}
...
async fn e2e_hook_contracts() {
    ...
    std::env::set_var("CODERUN_REPO_STATE", "e2e-hook-contract-head");

    // ── Seed TWO repos into the shared index ─────────────────────────────
    let repo_eshop = temp_dir("eshop");
    let repo_coderun = temp_dir("coderun");
    index_repo(
        &repo_eshop,
        "checkout.cs",
    ...
    index_repo(
        &repo_coderun,
        "router.rs",
        "// coderun daemon router daemon_marker_beta\npub fn route_request() {}",
    );

    // ── Build the engine as a daemon STARTED IN repo_coderun would ───────
    ...
    // ── (c) PreGeneration scoped to eShopOnWeb must NOT leak coderun paths ──
    ...
    let _ = std::fs::remove_dir_all(&repo_coderun);
