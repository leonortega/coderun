//! MCP (Model Context Protocol) surface for the Coderun daemon.
//!
...
//!
//! Purpose — the "no conversions" path for client plugins (opencode-coderun): instead
    ...
    match req.method.as_str() {
        "initialize" => result_response(
            ...
            json!({
                ...
                "capabilities": { "tools": {} },
                "serverInfo": { "name": "coderun", "version": env!("CARGO_PKG_VERSION") },
            }),
        ),
...
async fn handle_tools_call(
    ...
    match name {
        "coderun_context" => tool_context(state, id, args).await,
        "coderun_compress" => tool_compress(state, id, args),
        other => error_response(id, -32602, &format!("Unknown tool: {other}"), StatusCode::OK),
    }
}

/// `coderun_context` — build the enriched context answer for a prompt. Mirrors the
...
async fn tool_context(
    ...
    if !crate::metrics::global().is_ready() {
        tracing::warn!("MCP tools/call coderun_context rejected — daemon indexing in progress (-32001)");
        return error_response(
            id,
    ...
    .await
    {
        Ok(HttpResponsePayload::RewrittenMessage { rewritten, context_pack, .. }) => {
            ...
            let repository_state = context_pack
                ...
                .unwrap_or_default();
            tracing::info!(took_ms = %started.elapsed().as_millis(), "MCP coderun_context built");
            result_response(
                id,
                tool_result(
                    rewritten,
        ...
        Ok(HttpResponsePayload::OriginalPassthrough { original, reason }) => {
            ...
            tracing::info!(reason = %reason, "MCP coderun_context passthrough");
            result_response(
                id,
                tool_result(
                    original,
        ...
        Err(e) => {
            ...
            tracing::error!(error = %e, "MCP coderun_context build failed");
            result_response(
                id,
                tool_result(
                    format!("coderun context build failed: {e}"),
...
/// `coderun_compress` — the same optimizer pass `/hook` ToolOutput runs.
fn tool_compress(state: HttpServerState, id: Option<Value>, args: Value) -> (StatusCode, Json<Value>) {
    ...
    let output_type = match args.get("output_type").and_then(|v| v.as_str()) {
        Some("FileRead") => coderun_core::OutputType::FileRead,
        Some("SearchResult") => coderun_core::OutputType::SearchResult,
        Some("ShellOutput") => coderun_core::OutputType::ShellOutput,
        _ => coderun_core::OutputType::Other,
    };
    ...
    match handle_pre_tool_call(
        ...
        Ok(HttpResponsePayload::CompressedOutput { compressed, original_tokens, compressed_tokens, .. }) => {
            tracing::info!(tool = %tool_name, original_tokens = %original_tokens, compressed_tokens = %compressed_tokens, "MCP coderun_compress done");
            result_response(
                id,
                tool_result(
                    compressed,
        ...
        Err(e) => {
            ...
            result_response(
                id,
                tool_result(
                    format!("coderun compress failed: {e}"),
                    json!({ "type": "compress", "original_tokens": 0, "compressed_tokens": 0 }),
                    true,
                ),
            )
...
mod tests {
    ...
    fn empty_state() -> HttpServerState {
        let dir = std::env::temp_dir().join(format!("coderun_mcp_ut_{}", uuid::Uuid::new_v4()));
        ...
        HttpServerState {
            ...
            optimizer: Arc::new(ExecutionOptimizer::new(coderun_optimizer::OptimizerConfig::default())),
        }
    }
    ...
    async fn test_initialize() {
        ...
        assert_eq!(r["serverInfo"]["name"], "coderun");
    ...
    async fn test_tools_list() {
        ...
        assert!(names.contains(&"coderun_context"));
        assert!(names.contains(&"coderun_compress"));
    }
    ...
    async fn test_context_tool_readiness_gate() {
        ...
        let (status, resp) = handle_mcp(
            State(state),
            r#"{"jsonrpc":"2.0","id":9,"method":"tools/call","params":{"name":"coderun_context","arguments":{"prompt":"hi"}}}"#.to_string(),
        )
        .await;
    ...
    async fn test_compress_tool_ok() {
        ...
        let (status, resp) = handle_mcp(
            State(state),
            format!(
                r#"{{"jsonrpc":"2.0","id":10,"method":"tools/call","params":{{"name":"coderun_compress","arguments":{{"content":{}, "tool_name":"bash", "output_type":"ShellOutput"}}}}}}"#,
    ...
    async fn test_unknown_tool() {
        ...
        let (status, resp) = handle_mcp(
            State(state),
            r#"{"jsonrpc":"2.0","id":11,"method":"tools/call","params":{"name":"coderun_bogus","arguments":{}}}"#.to_string(),
        )
        .await;
        assert_eq!(status, StatusCode::OK);
