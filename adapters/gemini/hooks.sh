#!/usr/bin/env bash
# Knocode adapter for Gemini CLI (Tier 1 — UserPromptSubmit + PreToolUse analogs, v0.4.0)
...
# See .claude/hooks/knocode-pregeneration.sh for reference.

set -euo pipefail
KNOCODE_SOCKET="${KNOCODE_SOCKET:-/tmp/knocode.sock}"
KNOCODE_URL="${KNOCODE_DAEMON_URL:-http://127.0.0.1:9527}"
TIMEOUT="${KNOCODE_TIMEOUT_MS:-30000}"
...
if [ -S "$KNOCODE_SOCKET" ] && command -v python3 >/dev/null 2>&1; then
  :
fi
...
_ready_budget_ms="${KNOCODE_READY_TIMEOUT_MS:-10000}"
_ready_deadline=$(( $(date +%s) + (_ready_budget_ms + 999) / 1000 ))
while [ "$(date +%s)" -lt "$_ready_deadline" ]; do
  if _health_body="$(curl -sS --connect-timeout 1 --max-time 2 "$KNOCODE_URL/health" 2>/dev/null)"; then
...
RESPONSE="$(curl -sS --max-time $((TIMEOUT/1000)) -H 'Content-Type: application/json' -d "$HTTP_PAYLOAD" "$KNOCODE_URL/hook" 2>/dev/null || echo "")"
