...
A comprehensive catalog of code smells organized by family. Each smell includes a description, detection heuristics, and
the named refactorings that fix it.
...
A code smell is a surface indication that usually corresponds to a deeper structural problem. Smells are not bugs -- the
code works correctly -- but they make the code harder to understand, extend, and maintain. The term was coined by Kent
Beck and popularized by Martin Fowler.
...
**Description:** A method that tries to do too much. The longer a method is, the harder it is to understand, test, and
reuse.
...
Each comment block should be its own method: `validateOrder()`, `calculateTotals()`, `applyDiscounts()`, `saveOrder()`,
`sendConfirmation()`.

### Large Class

**Description:** A class that has too many fields, too many methods, or too many responsibilities. Often called a "God
Class" or "Blob."
...
**Description:** Groups of variables that appear together in multiple places -- method parameters, field declarations,
or local variables.
...
**Description:** The same switch/case or if/else chain on a type code appears in multiple places. When a new type is
added, every switch must be updated.
...
**Description:** A subclass inherits methods or data it does not want. It overrides parent methods to do nothing or
throws "not supported" exceptions.
...
**Description:** Two classes do essentially the same job but have different method names and signatures, preventing
interchangeability.
...
**Description:** One class changes for multiple unrelated reasons. It is the opposite of the Single Responsibility
Principle.
...
**Description:** A single logical change requires edits in many different classes. It is the opposite of Divergent
Change.
...
**Description:** A class that does too little to justify its existence. Each class costs complexity; if it doesn't carry
its weight, merge it.
...
**Description:** Code that is never executed -- unreachable branches, unused variables, unneeded parameters, methods no
one calls.
...
**Description:** Abstractions, parameters, hooks, or classes created "in case we need them someday." YAGNI -- You Aren't
Gonna Need It.
...
**Description:** The same or nearly identical code structure appears in more than one place. The most common and most
expensive smell.
...
**Description:** A method that uses more features (fields and methods) of another class than its own. It "envies" the
other class's data.
...
**Description:** Two classes are overly entangled -- accessing each other's private details, forming a bidirectional
dependency.
...
**Description:** A client asks object A for B, then asks B for C, then asks C for D: `a.getB().getC().getD()`. The
client is coupled to the entire navigation structure.
