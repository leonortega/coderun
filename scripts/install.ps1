...
# Only global plugin 'coderun' (never local .opencode/plugins) - embedded so repo file can be deleted
$globalPluginDir = "$env:USERPROFILE\.config\opencode\plugins"
$globalPlugin = Join-Path $globalPluginDir "coderun.ts"
New-Item -ItemType Directory -Force -Path $globalPluginDir | Out-Null
if (Test-Path $globalPlugin) {
  Ok "opencode plugin 'coderun' at global"
} else {
  # Create global plugin directly (embedded, no source file needed in repo)
  $pluginContent = @'
// Coderun plugin for opencode - generated by installer (global only)
import type { Plugin } from "@opencode-ai/plugin"
const CODERUN_DAEMON_URL = process.env.CODERUN_DAEMON_URL || "http://127.0.0.1:9527"
const REQUEST_TIMEOUT_MS = 30000
async function callCoderunDaemon(request: any): Promise<any | null> {
  try {
    const controller = new AbortController()
    const timeout = setTimeout(() => controller.abort(), REQUEST_TIMEOUT_MS)
    const response = await fetch(`${CODERUN_DAEMON_URL}/hook`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(request), signal: controller.signal })
    clearTimeout(timeout)
    if (!response.ok) { console.error(`[coderun] Daemon returned ${response.status}`); return null }
    return await response.json()
  } catch (error) { console.error(`[coderun] Daemon unreachable: ${error}`); return null }
}
function getOutputType(toolName: string): string {
  switch (toolName.toLowerCase()) {
    case "read": case "readfile": return "FileRead"
    case "grep": case "search": return "SearchResult"
    case "bash": case "shell": case "exec": return "ShellOutput"
    default: return "Other"
  }
}
export const coderun: Plugin = async ({ project, client, $, directory, worktree }) => {
  ...
  return {
    "message.updated": async (input: any, output: any) => {
      ...
      const request = { hook_type: "PreGeneration", payload: { type: "MessageRewrite", session_id: input.session_id || "unknown", message: message } }
      const response = await callCoderunDaemon(request)
      if (response?.payload.type === "RewrittenMessage" && response.payload.rewritten) {
        if (input.message) { input.message.content = response.payload.rewritten }
    ...
    "tool.execute.before": async (input: any, output: any) => {
      ...
      const request = { hook_type: "PreToolCall", payload: { type: "ToolOutput", tool_name: toolName, output_type: getOutputType(toolName), content: content } }
      const response = await callCoderunDaemon(request)
      if (response?.payload.type === "CompressedOutput" && response.payload.compressed) {
        if (output) { output.result = response.payload.compressed }
        console.log(`[coderun] Compressed ${toolName} output`)
      }
    },
  }
}
...
'@
  try { Set-Content -LiteralPath $globalPlugin -Value $pluginContent -Encoding UTF8; Ok "opencode plugin 'coderun' installed to global (embedded)" } catch { Warn "failed to create global plugin 'coderun': $_" }
}
if (Test-Path $globalPlugin) { Info "Restart opencode to load plugin 'coderun' (global, hooks: message.updated + tool.execute.before, daemon http://127.0.0.1:9527, 30s fail-open)" }
