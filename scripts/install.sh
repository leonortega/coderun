...
PLUGIN_EOF
  if [ -f "$GLOBAL_PLUGIN" ]; then ok "opencode plugin 'coderun' installed to global (embedded)"; else info "  global plugin install skipped"; fi
fi
if [ -f "$GLOBAL_PLUGIN" ]; then info "Restart opencode to load plugin 'coderun' (global, hooks: message.updated + tool.execute.before, daemon http://127.0.0.1:9527)"; fi
